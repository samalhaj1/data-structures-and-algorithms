...
Write a function named childSorting that sorts the characters below by the number of child in each house (fewest to most). If a house has the same number of child, sort alphabetically by house name.
...
let characters = [
  {
    name: 'Eddard',
    spouse: 'Catelyn',
    child: ['Robb', 'Sansa', 'Arya', 'Bran', 'Rickon'],
    house: 'Stark'
  },
  {
    name: 'Jon A.',
  ...
  },
  {
    name: 'Cersei',
    spouse: 'Robert',
    child: ['Joffrey', 'Myrcella', 'Tommen'],
    house: 'Lannister'
  },
  {
    name: 'Daenarys',
    spouse: 'Khal Drogo',
    child: ['Drogon', 'Rhaegal', 'Viserion'],
    house: 'Targaryen'
  },
  {
    name: 'Mace',
    spouse: 'Alerie',
    child: ['Margaery', 'Loras'],
    house: 'Tyrell'
  },
  {
    name: 'Euron',
...
const isCapitalized = (str) => {
  // Solution code here...
  let capitalLetter = /\b[A-Z]\w*/g;
  return str.match(capitalLetter) || [];
};
...
describe('Testing challenge 1', () => {
  test('It should sort the characters by number of child', () => {
    expect(childSorting(characters)[0].name).toStrictEqual('Euron');
    expect(childSorting(characters)[0].child.length).toStrictEqual(0);
  });
});
