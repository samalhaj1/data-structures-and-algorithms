...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  let starWarsArry = starWarsArr.sort((a, b) => Number(a.height) > Number(b.height) ? -1 : 1);
  return starWarsArry;
};
...
const listFoods = (recipe) => {
  ...
  //const comp = recipe.crisp;
  // for (let i = 0; i < comp.length; i++) {
  //let j = comp[i].indexOf(' ') + 1;
  //let upToDate = comp[i].slice(j);
  ///let index = upToDate.indexOf(' ') + 1;
  //result.push(upToDate.slice(index));
  //}
  return result;
};
